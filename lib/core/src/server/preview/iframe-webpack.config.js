...
export default ({
  ...
  entries.forEach((entryFilename) => {
    ...
    if (match) {
      ...
      const clientApi = require.resolve('@storybook/client-api');
      const clientLogger = require.resolve('@storybook/client-logger');

      virtualModuleMapping[entryFilename] = `
        const { addDecorator, addParameters, addParameterEnhancer } = require(${JSON.stringify(
          clientApi
        )})
        const { logger } = require(${JSON.stringify(clientLogger)})
